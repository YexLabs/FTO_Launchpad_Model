...
contract HenloDexPair is HenloDexERC20 {
    ...
    modifier lock() {
        require(unlocked == 1, "HenloDex: LOCKED");
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _mintFee(
        ...
        address feeTo = IHenloDexFactory(factory).feeTo();
    ...
    // this low-level function should be called from a contract which performs important safety checks
    function unbalance_mint(address to) external lock returns (uint liquidity) {
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        uint balance0 = IERC20(token0).balanceOf(address(this));
        uint balance1 = IERC20(token1).balanceOf(address(this));
        uint amount0 = balance0.sub(_reserve0);
    ...
    function burn(
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "HenloDex: INSUFFICIENT_LIQUIDITY_BURNED"
        );
    ...
    function swap(
        ...
        require(
            ...
            "HenloDex: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(
            amount0Out < _reserve0 && amount1Out < _reserve1,
            "HenloDex: INSUFFICIENT_LIQUIDITY"
        );
        ...
        uint balance1;
        {
            ...
            require(to != _token0 && to != _token1, "HenloDex: INVALID_TO");
            if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
            if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
            if (data.length > 0)
                IHenloDexCallee(to).uniswapV2Call(
                    msg.sender,
                    amount0Out,
                    amount1Out,
                    data
        ...
        require(
            ...
            "HenloDex: INSUFFICIENT_INPUT_AMOUNT"
        );
        {
