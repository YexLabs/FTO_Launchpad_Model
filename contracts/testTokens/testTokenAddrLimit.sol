...
contract testTokenAddrLimit is ERC20 {
    ...
    function faucet() public {
        require(
            !faucetedList[msg.sender],
            "You only can claim one time per address."
        );
        faucetedList[msg.sender] = true;
        _mint(msg.sender, _faucetAmount);
    }

    function isFauceted(address _address) public view returns (bool) {
        return !faucetedList[_address];
    }
